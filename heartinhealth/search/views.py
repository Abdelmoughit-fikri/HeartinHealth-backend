...
class searcHinhViewSet(ViewSet):
    def list(self, request):
        ...
        ###
        CI_results = CiArticle.objects.annotate(author_full_name=Concat('author__first_name', Value(' '), 'author__last_name')).filter(
            Q(title__icontains=query) |
            Q(search_queries__icontains=query) |
            Q(author__first_name__icontains=query) |
            Q(author__last_name__icontains=query) |
        ...
        CSD_results = CsdArticle.objects.annotate(
            author_full_name=Concat('author__first_name', Value(' '), 'author__last_name')).filter(
                Q(title__icontains=query) |
                Q(search_queries__icontains=query) |
                Q(author__first_name__icontains=query) |
                Q(author__last_name__icontains=query) |
